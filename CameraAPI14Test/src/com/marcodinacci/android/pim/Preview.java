...
public class Preview extends SurfaceView implements SurfaceHolder.Callback {
	...
	public Preview(Context context,String ip_addr) {
		super(context);
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		if(mMotionDetectionActive) {
			mCameraCallback = new CameraCallback(mContext, mCamera,mHandler,ip_addr);
...
final class CameraCallback implements Camera.PreviewCallback, 
	...
	//private final String PICTURE_PREFIX = "/DCIM//";
	private static final int PICTURE_DELAY = 600;
	TcpThread tcpChat;
	...
	Context ct;
	int port=12345;
	public CameraCallback(Context ct, Camera camera,Handler mHandler,String ip_addr) {
...
//		int serverResponseCode=0;
		...
		File f = new File("/sdcard/DCIM/",pictureName);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				Message msg = mHandler.obtainMessage(BluetoothChat.CAMERA);
		...
		*/
		tcpChat = new TcpThread(ip_addr,port);
		tcpChat.execute("kj");

		Log.e(TAG,"SENT CHATTING!!");
	}
